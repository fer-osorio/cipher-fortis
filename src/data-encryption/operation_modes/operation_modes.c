 ...
 * */
static void encryptXorBlockBytes(const uint8_t*const input, const KeyExpansion* ke_p, const uint8_t* XORsource, uint8_t* output){
  Block* buffer;
  blockFromBytes(input, buffer);
...
/*
 * Implements encryptXorBlockBytes on ioh object.
 * */
static void InputOutputHandlerEncryptXorBlockBytes(const struct InputOutputHandler* ioh, const uint8_t *XORsource, const KeyExpansion* ke_p){
  encryptXorBlockBytes(ioh->inputCurrentPossition, ke_p, XORsource, ioh->outputCurrentPossition);
}
...
static void encryptCBC__(const KeyExpansion* ke_p, const uint8_t* IV,struct InputOutputHandler* ioh){
  ...
  InputOutputHandlerEncryptXorBlockBytes(ioh, IV, ke_p);                        // -Encryption of the first block.
  for(size_t i = 1; i < ioh->sizeInBlocks; i++) {                               // -Encryption of the rest of the blocks.
    inputPreviousBlock = ioh->inputCurrentPossition;
    InputOutputHandlerMoveForwardOneBlock(ioh);
    InputOutputHandlerEncryptXorBlockBytes(ioh, inputPreviousBlock, ke_p);
  }
