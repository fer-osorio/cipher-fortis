 ...
 * };
 * EncryptionTester tester(compare, build, compare, build, callbacks);
 ...
 * @code
 * EncryptionTester tester(compare, build, compare, build);  // No callbacks
 * MyKE* ke = allocateKE();
 ...
 */
/**
 * @brief Memory management callbacks for C-style structures
 *
...
struct MemoryCallbacks {
    /// Allocates and initializes a KeyExpansion structure for given key size (in bits)
    std::function<KE*(size_t keySizeBits)> allocateKeyExpansion;

    /// Frees a KeyExpansion structure (should accept pointer to pointer and nullify)
    std::function<void(KE**)> freeKeyExpansion;

    /// Allocates and initializes a Block structure (always 16 bytes for AES)
    std::function<BT*()> allocateBlock;

    /// Frees a Block structure (should accept pointer to pointer and nullify)
    std::function<void(BT**)> freeBlock;

    /**
     * @brief Validates that all callbacks are properly set
     * @return true if all callbacks are non-null, false otherwise
     */
    bool isValid() const {
        return allocateKeyExpansion && freeKeyExpansion &&
...
public:
    ...
    bool validateMemoryCallbacks() {
        ...
        KeyExpansionType* ke128 = memoryCallbacks_.allocateKeyExpansion(128);
        KeyExpansionType* ke256 = memoryCallbacks_.allocateKeyExpansion(256);
        ...
        // Clean up
        if (ke128) memoryCallbacks_.freeKeyExpansion(&ke128);
        if (ke256) memoryCallbacks_.freeKeyExpansion(&ke256);
        if (b1) memoryCallbacks_.freeBlock(&b1);
        if (b2) memoryCallbacks_.freeBlock(&b2);
    ...
    */
    bool runTestSuite(
        ...
        KeyExpansionType* keBuffer = memoryCallbacks_.allocateKeyExpansion(keySizeBits);
        BlockType* inputBuffer = memoryCallbacks_.allocateBlock();
        BlockType* outputBuffer = memoryCallbacks_.allocateBlock();
        BlockType* tempBuffer = memoryCallbacks_.allocateBlock();

        // Guard against allocation failures
        if (!keBuffer || !inputBuffer || !outputBuffer || !tempBuffer) {
            // Clean up any successful allocations
            if (keBuffer) memoryCallbacks_.freeKeyExpansion(&keBuffer);
            if (inputBuffer) memoryCallbacks_.freeBlock(&inputBuffer);
            if (outputBuffer) memoryCallbacks_.freeBlock(&outputBuffer);
            if (tempBuffer) memoryCallbacks_.freeBlock(&tempBuffer);
        ...
        try {
            ...
            memoryCallbacks_.freeKeyExpansion(&keBuffer);
            memoryCallbacks_.freeBlock(&inputBuffer);
            memoryCallbacks_.freeBlock(&outputBuffer);
            memoryCallbacks_.freeBlock(&tempBuffer);
            throw;
        }
