/**
 * @file fips197_key_expansion.cpp
 * @brief Implementation of FIPS 197 Appendix A - Key Expansion Test Vectors
 */

#include "../../include/test-vectors/fips197_key_expansion.hpp"
    ...
    namespace AES {
        namespace FIPS197 {
            namespace KeyExpansion {
                ...
                // =========================================================================
                // Expanded Key Data Implementation
                // =========================================================================

                // AES-128 Expanded Key (176 bytes = 44 words × 4 bytes)
                const unsigned char AES128_Expanded[176] = {
                ...
                // AES-192 Expanded Key (208 bytes = 52 words × 4 bytes)
                const unsigned char AES192_Expanded[208] = {
                ...
                // AES-256 Expanded Key (240 bytes = 60 words × 4 bytes)
                const unsigned char AES256_Expanded[240] = {
                ...
                const unsigned char* getExpandedKey(KeySize ks) {
                    switch(ks) {
                        case KeySize::AES128: return AES128_Expanded;
                        case KeySize::AES192: return AES192_Expanded;
                        case KeySize::AES256: return AES256_Expanded;
                ...
                TestVector::TestVector(KeySize ks) {
                    keySize_ = ks;
                    direction_ = Direction::Encrypt; // Key expansion is direction-agnostic
                    dataSource_ = DataSource::NIST_Official;
                    
                    // Use the standard NIST keys from Keys namespace
                    key_ = Keys::NIST::get(ks);
                ...
                const std::vector<unsigned char> TestVector::getExpectedExpansion() const {
                    ...
                    size_t size = getExpandedKeySizeBytes(keySize_);
                    return std::vector<unsigned char>(expectedExpansion_, expectedExpansion_ + size);
                }
                ...
                std::unique_ptr<TestVector> create(KeySize ks) {
                    return std::make_unique<TestVector>(ks);
                }
