...
static void encryptBlock(const Block* input, const Block keyExpansion[], size_t Nk, Block* output, bool debug) {
  size_t i, j;
  size_t Nr = Nk+6;
  ...
  transposeBlock(input, output);                                                // Transpose block before encryption
...
void decryptBlock(Block* input, const Block keyExpansion[], size_t Nk, Block* output) {
  size_t Nr = Nk+6;
  size_t i = Nr;
  AddRoundKey(input, keyExpansion, i);
  for(i--; i > 0; i--) {
    InvShiftRows(input);
    InvSubBytes(input);
    AddRoundKey(input, keyExpansion, i);
  ...
  InvShiftRows(input);
  InvSubBytes(input);
  AddRoundKey(input,keyExpansion, 0);
