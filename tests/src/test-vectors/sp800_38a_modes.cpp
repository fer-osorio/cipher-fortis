/**
 * @file sp800_38a_modes.cpp
 * @brief Implementation of NIST SP 800-38A Appendix F - Modes of Operation Test Vectors
 */

#include "../../include/test-vectors/sp800_38a_modes.hpp"
#include "../../include/test-vectors/keys.hpp"

namespace TestVectors {
    namespace AES {
        namespace SP800_38A {

            // =========================================================================
            // Common Test Data Implementation
            // =========================================================================

            const unsigned char kPlainText[kDataSize] = {
                0x6b, 0xc1, 0xbe, 0xe2, 0x2e, 0x40, 0x9f, 0x96, 0xe9, 0x3d, 0x7e, 0x11, 0x73, 0x93, 0x17, 0x2a,
                0xae, 0x2d, 0x8a, 0x57, 0x1e, 0x03, 0xac, 0x9c, 0x9e, 0xb7, 0x6f, 0xac, 0x45, 0xaf, 0x8e, 0x51,
                0x30, 0xc8, 0x1c, 0x46, 0xa3, 0x5c, 0xe4, 0x11, 0xe5, 0xfb, 0xc1, 0x19, 0x1a, 0x0a, 0x52, 0xef,
                0xf6, 0x9f, 0x24, 0x45, 0xdf, 0x4f, 0x9b, 0x17, 0xad, 0x2b, 0x41, 0x7b, 0xe6, 0x6c, 0x37, 0x10
            };

            const unsigned char kInitializationVector[16] = {
                0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07,
                0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f
            };

            const unsigned char kInitialCounter[16] = {
                0xf0, 0xf1, 0xf2, 0xf3, 0xf4, 0xf5, 0xf6, 0xf7,
                0xf8, 0xf9, 0xfa, 0xfb, 0xfc, 0xfd, 0xfe, 0xff
            };

            const unsigned char* getIV() {
                return kInitializationVector;
            }

            std::vector<unsigned char> getIVAsVector() {
                return std::vector<unsigned char>(kInitializationVector, kInitializationVector + 16);
            }

            const unsigned char* getCounter() {
                return kInitialCounter;
            }

            std::vector<unsigned char> getCounterAsVector() {
                return std::vector<unsigned char>(kInitialCounter, kInitialCounter + 16);
            }

            // =========================================================================
            // Base Test Vector Class Implementation
            // =========================================================================

            std::vector<unsigned char> ModeTestVectorBase::getInputAsVector() const {
                return std::vector<unsigned char>(input_, input_ + kDataSize);
            }

            std::vector<unsigned char> ModeTestVectorBase::getExpectedOutputAsVector() const {
                return std::vector<unsigned char>(expectedOutput_, expectedOutput_ + kDataSize);
            }

            // =========================================================================
            // ECB Mode Implementation
            // =========================================================================

            namespace ECB {

                const unsigned char AES128_CipherText[kDataSize] = {
                    0x3a, 0xd7, 0x7b, 0xb4, 0x0d, 0x7a, 0x36, 0x60, 0xa8, 0x9e, 0xca, 0xf3, 0x24, 0x66, 0xef, 0x97,
                    0xf5, 0xd3, 0xd5, 0x85, 0x03, 0xb9, 0x69, 0x9d, 0xe7, 0x85, 0x89, 0x5a, 0x96, 0xfd, 0xba, 0xaf,
                    0x43, 0xb1, 0xcd, 0x7f, 0x59, 0x8e, 0xce, 0x23, 0x88, 0x1b, 0x00, 0xe3, 0xed, 0x03, 0x06, 0x88,
                    0x7b, 0x0c, 0x78, 0x5e, 0x27, 0xe8, 0xad, 0x3f, 0x82, 0x23, 0x20, 0x71, 0x04, 0x72, 0x5d, 0xd4
                };

                const unsigned char AES192_CipherText[kDataSize] = {
                    0xbd, 0x33, 0x4f, 0x1d, 0x6e, 0x45, 0xf2, 0x5f, 0xf7, 0x12, 0xa2, 0x14, 0x57, 0x1f, 0xa5, 0xcc,
                    0x97, 0x41, 0x04, 0x84, 0x6d, 0x0a, 0xd3, 0xad, 0x77, 0x34, 0xec, 0xb3, 0xec, 0xee, 0x4e, 0xef,
                    0xef, 0x7a, 0xfd, 0x22, 0x70, 0xe2, 0xe6, 0x0a, 0xdc, 0xe0, 0xba, 0x2f, 0xac, 0xe6, 0x44, 0x4e,
                    0x9a, 0x4b, 0x41, 0xba, 0x73, 0x8d, 0x6c, 0x72, 0xfb, 0x16, 0x69, 0x16, 0x03, 0xc1, 0x8e, 0x0e
                };

                const unsigned char AES256_CipherText[kDataSize] = {
                    0xf3, 0xee, 0xd1, 0xbd, 0xb5, 0xd2, 0xa0, 0x3c, 0x06, 0x4b, 0x5a, 0x7e, 0x3d, 0xb1, 0x81, 0xf8,
                    0x59, 0x1c, 0xcb, 0x10, 0xd4, 0x10, 0xed, 0x26, 0xdc, 0x5b, 0xa7, 0x4a, 0x31, 0x36, 0x28, 0x70,
                    0xb6, 0xed, 0x21, 0xb9, 0x9c, 0xa6, 0xf4, 0xf9, 0xf1, 0x53, 0xe7, 0xb1, 0xbe, 0xaf, 0xed, 0x1d,
                    0x23, 0x30, 0x4b, 0x7a, 0x39, 0xf9, 0xf3, 0xff, 0x06, 0x7d, 0x8d, 0x8f, 0x9e, 0x24, 0xec, 0xc7
                };

                const unsigned char* getCipherText(KeySize ks) {
                    switch(ks) {
                        case KeySize::AES128: return AES128_CipherText;
                        case KeySize::AES192: return AES192_CipherText;
                        case KeySize::AES256: return AES256_CipherText;
                        default: return nullptr;
                    }
                }

                TestVector::TestVector(KeySize ks, Direction dir) {
                    keySize_ = ks;
                    direction_ = dir;
                    dataSource_ = DataSource::NIST_Official;
                    mode_ = CipherMode::ECB;
                    key_ = Keys::NIST::get(ks);

                    switch(dir) {
                        case Direction::Encrypt:
                            input_ = kPlainText;
                            expectedOutput_ = getCipherText(ks);
                            break;
                        case Direction::Decrypt:
                            input_ = getCipherText(ks);
                            expectedOutput_ = kPlainText;
                            break;
                    }
                }

                std::unique_ptr<TestVector> create(KeySize ks, Direction dir) {
                    return std::make_unique<TestVector>(ks, dir);
                }

            } // namespace ECB

            // =========================================================================
            // CBC Mode Implementation
            // =========================================================================

            namespace CBC {

                const unsigned char AES128_CipherText[kDataSize] = {
                    0x76, 0x49, 0xab, 0xac, 0x81, 0x19, 0xb2, 0x46, 0xce, 0xe9, 0x8e, 0x9b, 0x12, 0xe9, 0x19, 0x7d,
                    0x50, 0x86, 0xcb, 0x9b, 0x50, 0x72, 0x19, 0xee, 0x95, 0xdb, 0x11, 0x3a, 0x91, 0x76, 0x78, 0xb2,
                    0x73, 0xbe, 0xd6, 0xb8, 0xe3, 0xc1, 0x74, 0x3b, 0x71, 0x16, 0xe6, 0x9e, 0x22, 0x22, 0x95, 0x16,
                    0x3f, 0xf1, 0xca, 0xa1, 0x68, 0x1f, 0xac, 0x09, 0x12, 0x0e, 0xca, 0x30, 0x75, 0x86, 0xe1, 0xa7
                };

                const unsigned char AES192_CipherText[kDataSize] = {
                    0x4f, 0x02, 0x1d, 0xb2, 0x43, 0xbc, 0x63, 0x3d, 0x71, 0x78, 0x18, 0x3a, 0x9f, 0xa0, 0x71, 0xe8,
                    0xb4, 0xd9, 0xad, 0xa9, 0xad, 0x7d, 0xed, 0xf4, 0xe5, 0xe7, 0x38, 0x76, 0x3f, 0x69, 0x14, 0x5a,
                    0x57, 0x1b, 0x24, 0x20, 0x12, 0xfb, 0x7a, 0xe0, 0x7f, 0xa9, 0xba, 0xac, 0x3d, 0xf1, 0x02, 0xe0,
                    0x08, 0xb0, 0xe2, 0x79, 0x88, 0x59, 0x88, 0x81, 0xd9, 0x20, 0xa9, 0xe6, 0x4f, 0x56, 0x15, 0xcd
                };

                const unsigned char AES256_CipherText[kDataSize] = {
                    0xf5, 0x8c, 0x4c, 0x04, 0xd6, 0xe5, 0xf1, 0xba, 0x77, 0x9e, 0xab, 0xfb, 0x5f, 0x7b, 0xfb, 0xd6,
                    0x9c, 0xfc, 0x4e, 0x96, 0x7e, 0xdb, 0x80, 0x8d, 0x67, 0x9f, 0x77, 0x7b, 0xc6, 0x70, 0x2c, 0x7d,
                    0x39, 0xf2, 0x33, 0x69, 0xa9, 0xd9, 0xba, 0xcf, 0xa5, 0x30, 0xe2, 0x63, 0x04, 0x23, 0x14, 0x61,
                    0xb2, 0xeb, 0x05, 0xe2, 0xc3, 0x9b, 0xe9, 0xfc, 0xda, 0x6c, 0x19, 0x07, 0x8c, 0x6a, 0x9d, 0x1b
                };

                const unsigned char* getCipherText(KeySize ks) {
                    switch(ks) {
                        case KeySize::AES128: return AES128_CipherText;
                        case KeySize::AES192: return AES192_CipherText;
                        case KeySize::AES256: return AES256_CipherText;
                        default: return nullptr;
                    }
                }

                TestVector::TestVector(KeySize ks, Direction dir) {
                    keySize_ = ks;
                    direction_ = dir;
                    dataSource_ = DataSource::NIST_Official;
                    mode_ = CipherMode::CBC;
                    key_ = Keys::NIST::get(ks);
                    iv_ = kInitializationVector;

                    switch(dir) {
                        case Direction::Encrypt:
                            input_ = kPlainText;
                            expectedOutput_ = getCipherText(ks);
                            break;
                        case Direction::Decrypt:
                            input_ = getCipherText(ks);
                            expectedOutput_ = kPlainText;
                            break;
                    }
                }

                std::vector<unsigned char> TestVector::getIVAsVector() const {
                    return std::vector<unsigned char>(iv_, iv_ + 16);
                }

                std::unique_ptr<TestVector> create(KeySize ks, Direction dir) {
                    return std::make_unique<TestVector>(ks, dir);
                }

            } // namespace CBC

            // =========================================================================
            // OFB Mode Implementation
            // =========================================================================

            namespace OFB {

                const unsigned char AES128_CipherText[kDataSize] = {
                    0x3b, 0x3f, 0xd9, 0x2e, 0xb7, 0x2d, 0xad, 0x20, 0x33, 0x34, 0x49, 0xf8, 0xe8, 0x3c, 0xfb, 0x4a,
                    0x77, 0x89, 0x50, 0x8d, 0x16, 0x91, 0x8f, 0x03, 0xf5, 0x3c, 0x52, 0xda, 0xc5, 0x4e, 0xd8, 0x25,
                    0x97, 0x40, 0x05, 0x1e, 0x9c, 0x5f, 0xec, 0xf6, 0x43, 0x44, 0xf7, 0xa8, 0x22, 0x60, 0xed, 0xcc,
                    0x30, 0x4c, 0x65, 0x28, 0xf6, 0x59, 0xc7, 0x78, 0x66, 0xa5, 0x10, 0xd9, 0xc1, 0xd6, 0xae, 0x5e
                };

                const unsigned char AES192_CipherText[kDataSize] = {
                    0xcd, 0xc8, 0x0d, 0x6f, 0xdd, 0xf1, 0x8c, 0xab, 0x34, 0xc2, 0x59, 0x09, 0xc9, 0x9a, 0x41, 0x74,
                    0xfc, 0xc2, 0x8b, 0x8d, 0x4c, 0x63, 0x83, 0x7c, 0x09, 0xe8, 0x17, 0x00, 0xc1, 0x10, 0x04, 0x01,
                    0x8d, 0x9a, 0x9a, 0xea, 0xc0, 0xf6, 0x59, 0x6f, 0x55, 0x9c, 0x6d, 0x4d, 0xaf, 0x59, 0xa5, 0xf2,
                    0x6d, 0x9f, 0x20, 0x08, 0x57, 0xca, 0x6c, 0x3e, 0x9c, 0xac, 0x52, 0x4b, 0xd9, 0xac, 0xc9, 0x2a
                };

                const unsigned char AES256_CipherText[kDataSize] = {
                    0xdc, 0x7e, 0x84, 0xbf, 0xda, 0x79, 0x16, 0x4b, 0x7e, 0xcd, 0x84, 0x86, 0x98, 0x5d, 0x38, 0x60,
                    0x4f, 0xeb, 0xdc, 0x67, 0x40, 0xd2, 0x0b, 0x3a, 0xc8, 0x8f, 0x6a, 0xd8, 0x2a, 0x4f, 0xb0, 0x8d,
                    0x71, 0xab, 0x47, 0xa0, 0x86, 0xe8, 0x6e, 0xed, 0xf3, 0x9d, 0x1c, 0x5b, 0xba, 0x97, 0xc4, 0x08,
                    0x01, 0x26, 0x14, 0x1d, 0x67, 0xf3, 0x7b, 0xe8, 0x53, 0x8f, 0x5a, 0x8b, 0xe7, 0x40, 0xe4, 0x84
                };

                const unsigned char* getCipherText(KeySize ks) {
                    switch(ks) {
                        case KeySize::AES128: return AES128_CipherText;
                        case KeySize::AES192: return AES192_CipherText;
                        case KeySize::AES256: return AES256_CipherText;
                        default: return nullptr;
                    }
                }

                TestVector::TestVector(KeySize ks, Direction dir) {
                    keySize_ = ks;
                    direction_ = dir;
                    dataSource_ = DataSource::NIST_Official;
                    mode_ = CipherMode::OFB;
                    key_ = Keys::NIST::get(ks);
                    iv_ = kInitializationVector;

                    switch(dir) {
                        case Direction::Encrypt:
                            input_ = kPlainText;
                            expectedOutput_ = getCipherText(ks);
                            break;
                        case Direction::Decrypt:
                            input_ = getCipherText(ks);
                            expectedOutput_ = kPlainText;
                            break;
                    }
                }

                std::vector<unsigned char> TestVector::getIVAsVector() const {
                    return std::vector<unsigned char>(iv_, iv_ + 16);
                }

                std::unique_ptr<TestVector> create(KeySize ks, Direction dir) {
                    return std::make_unique<TestVector>(ks, dir);
                }

            } // namespace OFB

            // =========================================================================
            // CTR Mode Implementation
            // =========================================================================

            namespace CTR {

                const unsigned char AES128_CipherText[kDataSize] = {
                    0x87, 0x4d, 0x61, 0x91, 0xb6, 0x20, 0xe3, 0x26, 0x1b, 0xef, 0x68, 0x64, 0x99, 0x0d, 0xb6, 0xce,
                    0x98, 0x06, 0xf6, 0x6b, 0x79, 0x70, 0xfd, 0xff, 0x86, 0x17, 0x18, 0x7b, 0xb9, 0xff, 0xfd, 0xff,
                    0x5a, 0xe4, 0xdf, 0x3e, 0xdb, 0xd5, 0xd3, 0x5e, 0x5b, 0x4f, 0x09, 0x02, 0x0d, 0xb0, 0x3e, 0xab,
                    0x1e, 0x03, 0x1d, 0xda, 0x2f, 0xbe, 0x03, 0xd1, 0x79, 0x21, 0x70, 0xa0, 0xf3, 0x00, 0x9c, 0xee
                };

                const unsigned char AES192_CipherText[kDataSize] = {
                    0x1a, 0xbc, 0x93, 0x24, 0x17, 0x52, 0x1c, 0xa2, 0x4f, 0x2b, 0x04, 0x59, 0xfe, 0x7e, 0x6e, 0x0b,
                    0x09, 0x03, 0x39, 0xec, 0x0a, 0xa6, 0xfa, 0xef, 0xd5, 0xcc, 0xc2, 0xc6, 0xf4, 0xce, 0x8e, 0x94,
                    0x1e, 0x36, 0xb2, 0x6b, 0xd1, 0xeb, 0xc6, 0x70, 0xd1, 0xbd, 0x1d, 0x66, 0x56, 0x20, 0xab, 0xf7,
                    0x4f, 0x78, 0xa7, 0xf6, 0xd2, 0x98, 0x09, 0x58, 0x5a, 0x97, 0xda, 0xec, 0x58, 0xc6, 0xb0, 0x50
                };

                const unsigned char AES256_CipherText[kDataSize] = {
                    0x60, 0x1e, 0xc3, 0x13, 0x77, 0x57, 0x89, 0xa5, 0xb7, 0xa7, 0xf5, 0x04, 0xbb, 0xf3, 0xd2, 0x28,
                    0xf4, 0x43, 0xe3, 0xca, 0x4d, 0x62, 0xb5, 0x9a, 0xca, 0x84, 0xe9, 0x90, 0xca, 0xca, 0xf5, 0xc5,
                    0x2b, 0x09, 0x30, 0xda, 0xa2, 0x3d, 0xe9, 0x4c, 0xe8, 0x70, 0x17, 0xba, 0x2d, 0x84, 0x98, 0x8d,
                    0xdf, 0xc9, 0xc5, 0x8d, 0xb6, 0x7a, 0xad, 0xa6, 0x13, 0xc2, 0xdd, 0x08, 0x45, 0x79, 0x41, 0xa6
                };

                const unsigned char* getCipherText(KeySize ks) {
                    switch(ks) {
                        case KeySize::AES128: return AES128_CipherText;
                        case KeySize::AES192: return AES192_CipherText;
                        case KeySize::AES256: return AES256_CipherText;
                        default: return nullptr;
                    }
                }

                TestVector::TestVector(KeySize ks, Direction dir) {
                    keySize_ = ks;
                    direction_ = dir;
                    dataSource_ = DataSource::NIST_Official;
                    mode_ = CipherMode::CTR;
                    key_ = Keys::NIST::get(ks);
                    counter_ = kInitialCounter;

                    switch(dir) {
                        case Direction::Encrypt:
                            input_ = kPlainText;
                            expectedOutput_ = getCipherText(ks);
                            break;
                        case Direction::Decrypt:
                            input_ = getCipherText(ks);
                            expectedOutput_ = kPlainText;
                            break;
                    }
                }

                std::vector<unsigned char> TestVector::getCounterAsVector() const {
                    return std::vector<unsigned char>(counter_, counter_ + 16);
                }

                std::unique_ptr<TestVector> create(KeySize ks, Direction dir) {
                    return std::make_unique<TestVector>(ks, dir);
                }

            } // namespace CTR

            // =========================================================================
            // Generic Factory Implementation
            // =========================================================================

            std::unique_ptr<ModeTestVectorBase> create(KeySize ks, CipherMode mode, Direction dir) {
                switch(mode) {
                    case CipherMode::ECB:
                        return ECB::create(ks, dir);
                    case CipherMode::CBC:
                        return CBC::create(ks, dir);
                    case CipherMode::OFB:
                        return OFB::create(ks, dir);
                    case CipherMode::CTR:
                        return CTR::create(ks, dir);
                    default:
                        return nullptr;
                }
            }

        } // namespace SP800_38A
    } // namespace AES
} // namespace TestVectors
